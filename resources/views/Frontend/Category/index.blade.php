...
@section('title', 'Category Destination | '.ucwords($config->site_name))
@section('content')
            ...
            <div class="portfolio">
					<div class="row acurate">
						<div class="col-sm-12 acurate">
							<div class="filtering-button">
                                @foreach($typName as $typN)
                                    <h3>Data Of <u>{{$typN->type_name}}</u></h3>	
                                @endforeach						
							</div>
						</div>
                    </div>
                    ...
                    <div class="single-post">
                        <div class="inner-post">
                            <div class="post-img">
                                <a href="#"><img src="{{ url('images/destination/'. $data->pict) }}" alt="blog"></a>
                            </div>
                            <div class="post-info">
                                <div class="post-title">
                                    <h3><a href="{{ url('/destination/detail', $data->dest_id) }}">{{ $data->dest_name }}</a></h3>
                                </div>
                                <div class="post-content">
                                    <p>
                                        @php 
                                          echo "<p>".substr(strip_tags($data->desc), 0, 200). '...'."</p>" 
                                        @endphp
                                    </p>
                                </div>
                                <div class="blog-meta fix">
                                    ...
                                    <div class="post-readmore pull-right">
                                        <a href="{{ url('/destination/detail', $data->dest_id) }}" class="readmore-btn">Read More <span>+</span></a>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    @else
                        <div class="portfolio">
					        <div class="row acurate">
                                @foreach($typName as $typN)
                                </center><h1>No Destinatian at <u>{{$typN->type_name}}</u></h1>	</center>
                                @endforeach		
                            </div>
                        </div> 
                    @endif
